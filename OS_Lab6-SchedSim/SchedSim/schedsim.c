...
{ 
     ...
     int i, shortest, pcounter, current_time, comp_time = 0;
     int rem_bt[n];
     ...
     while (pcounter < n) 
     {
       ...
       if (rem_bt[shortest] == 0)
       {
         ...
         plist[shortest].wt = comp_time - plist[shortest].bt; // Calculate waiting time for each completed process. *wt[i]= Completion time - arrival_time-burst_time*
         rem_bt[shortest] = 9999; // Increment time lap by one.
       }
       current_time += 1;
     }
} 
